...
let replaceCSSComments = function (str) {
...
function checkEmail(str) {
  return (/^[a-z.?-?\S+]+[a-z]?@[a-z]+\.[a-z\S+]{2,3}\.*[a-z\S+]*$/).test(str);
}
...
function checkDomainUrl(str) {
  return (/^[a-z\S+]+[:/\\\\][\w]+[-._]?\.[a-z\S]{2,4}\.*[a-z]*\S+$/).test(str);
}
...
function createLinksFromDomains(str) {
  let reg = new RegExp('(http:|https:)\\/\\/(?!:\\/\\/)([a-zA-Z0-9-_]+\\.)*[a-zA-Z0-9][a-zA-Z0-9-_]+\\.[a-zA-Z]+', 'gi');
